...
if (obj_world_initialize_god.r_travel_distance <= 0 && obj_world_initialize_god.r_travel_distance != -1000) // add new platforms to right side
{
    ...
    global.temp_world_adjacency_path_list_position = ds_list_size(global.world_adjacency_path_list)
    ...
    if(ds_list_size(global.right_platform_list) != 0)
        obj_world_initialize_god.r_travel_distance = ds_list_find_value(ds_list_find_value(global.right_platform_list,0),0) - (obj_world_initialize_god.current_world_x + (pi*WORLD_RADIUS)) // and reset right travel distance
    else // there are no more platforms to the right
        obj_world_initialize_god.r_travel_distance = -1000 // so set to sentinel
    ds_list_add(global.path_destroy_list,number_of_paths_made)
}

if (ds_list_size(global.in_world_platform_list) != 0 && ds_list_find_value(ds_list_find_value(global.in_world_platform_list,0),0) < obj_world_initialize_god.current_world_x - (pi * WORLD_RADIUS))
{
    ...
    while(!path_exists(ds_list_find_value(ds_list_find_value(global.world_adjacency_path_list,0),2))) // if we've deleted the first path
    {
        ds_list_delete(global.world_adjacency_path_list,0) // delete the entry from the world adjacency list
        global.world_adjacency_path_list_current_index-- // decrement current index
    }
    
    obj_world_initialize_god.l_travel_distance = (obj_world_initialize_god.current_world_x - (pi*WORLD_RADIUS)) - ds_list_find_value(ds_list_find_value(global.left_platform_list,ds_list_size(global.left_platform_list)-1),0)

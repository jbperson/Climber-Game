...
while(!ds_stack_empty(modify_stack))
{
    ...
    modify_block = collision_point(modify_x,modify_y,obj_block_climbable,true,true)
...
if(!object_is_ancestor(test_next_block.object_index,obj_block_path_point_CtW_parent))
{
    with(test_next_block)
    {
        associated_path = global.temp_associated_path

        modify_path_from_append_next_path_point_block_climb()
    }

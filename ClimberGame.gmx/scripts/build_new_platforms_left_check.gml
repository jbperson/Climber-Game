...
if (l_travel_distance <= 0 && l_travel_distance != -1000) // add new platforms to right side
{
    ...
    if (ds_list_size(global.left_platform_list) != 0)
        l_travel_distance = (current_world_x - (pi*WORLD_RADIUS)) - ds_list_find_value(ds_list_find_value(global.left_platform_list,ds_list_size(global.left_platform_list) - 1),0) // and reset left travel distance
    else
...
if (ds_list_size(global.in_world_platform_list) != 0 && ds_list_find_value(ds_list_find_value(global.in_world_platform_list,ds_list_size(global.in_world_platform_list)-1),0) > current_world_x + (pi * WORLD_RADIUS))
{
    ...
    r_travel_distance = ds_list_find_value(ds_list_find_value(global.right_platform_list,0),0) - (current_world_x + (pi * WORLD_RADIUS))// get distance to travel right until reaching next platform. The '2' is where that data is stored in a zone lis
